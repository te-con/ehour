...
public class CustomerDaoHibernateImpl extends AbstractGenericDaoHibernateImpl<Customer, Integer> implements CustomerDao
{
	...
	public List<Customer> findAllCustomersHavingReporter(User user, boolean onlyActiveCustomers) {
		String query = onlyActiveCustomers ? "Customer.findAllActiveCustomersHavingReporter" : "Customer.findAllCustomersHavingReporter" ;
		
		return findByNamedQueryAndNamedParam(query, "userId", user.getUserId(), true, CACHEREGION);
	}

	@Override
	public List<Customer> findAllCustomersForWhichUserIsaReviewer(User user) {
		return findByNamedQueryAndNamedParam("Customer.findAllCustomersForWhichUserIsaReviewer", "userId", user.getUserId(), true, CACHEREGION);
	}
