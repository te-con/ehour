...
public class CustomerReviewerPanel extends AbstractAjaxPanel<ReportCriteria> {
    ...
    private void addAllComponents(List<ApprovalStatus> approvalStatuses) {
        ...
        List<IColumn<ApprovalStatus, Date>> columns = Lists.newArrayList();
        ...
        columns.add(new PropertyColumn<ApprovalStatus, Date>(new ResourceModel("customerreviewer.timesheet.column.user"), "user.fullName"));
        ...
        // LLI for Richemont
        // 20 to 50 items per page because next page display wrong information
        AjaxFallbackDefaultDataTable<ApprovalStatus, Date> table = new AjaxFallbackDefaultDataTable<ApprovalStatus, Date>("data", columns, new CustomerReviewerDataProvider(approvalStatuses), 50) {
            protected Item<ApprovalStatus> newRowItem(final String id, int index, final IModel<ApprovalStatus> model) {
                return new DataRowItem(id, index, model);
            }
        };
    ...
    private class DataRowItem extends Item<ApprovalStatus> {

        public DataRowItem(String id, int index, IModel<ApprovalStatus> model) {
