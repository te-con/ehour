...
public class PmReportPanel extends AbstractReportPanel
{
	...
	public PmReportPanel(String id, ProjectManagerReport report)
	{
		...
		blueBorder.add(new ListView<ActivityAggregateReportElement>("report", new ArrayList<ActivityAggregateReportElement>(report.getAggregates()))
		{
			private static final long serialVersionUID = 1L;

			@Override
			protected void populateItem(ListItem<ActivityAggregateReportElement> item)
			{
				ActivityAggregateReportElement aggregate = item.getModelObject();
				
				item.add(new Label("user", aggregate.getActivity().getAssignedUser().getFullName()));
//				item.add(new Label("role", "Project Manager"));
				item.add(new Label("booked", new Model<Float>(aggregate.getHours().floatValue())));
				item.add(new Label("allotted", new Model<Float>(aggregate.getActivity().getAllottedHours())));
