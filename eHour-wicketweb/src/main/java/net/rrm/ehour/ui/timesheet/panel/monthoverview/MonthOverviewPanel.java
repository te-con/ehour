...
public class MonthOverviewPanel extends Panel
{
	...
	private TooltipLabel createProjectCodeTooltip(TimesheetEntry entry)
	{
		...
		String description = entry.getEntryId().getActivity().getProject().getDescription();
		...
		tooltipText.append(entry.getEntryId().getActivity().getProject().getName());
		tooltipText.append("</em>");
		
		String projectCode = entry.getEntryId().getActivity().getProject().getProjectCode();
