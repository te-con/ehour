...
public class MonthlyApprovalPanel extends Panel {
	...
	private void addPanelComponents(Calendar overviewFor, SortedSet<UserProjectStatus> userProjectStatuses) {
		...
		CustomTitledGreyRoundedBorder greyBorder = new CustomTitledGreyRoundedBorder("greyBorder", new Label("title", new ResourceModel(
				"monthlyapproval.title")), WebGeo.W_CONTENT_MEDIUM);
		...
		ListView<Customer> customerListView = new ListView<Customer>("monthlyApprovalStatus", customers) {
			...
			protected void populateItem(final ListItem<Customer> item) {
				...
				Label appprovalStatusLabel = new Label("approvalStatus", approvalStatusService
						.getApprovalStatusForUserWorkingForCustomer(user, customer, monthRange).get(0).getStatus().toString());
				item.add(appprovalStatusLabel);
				...
				AjaxLink<Void> requestApprovalLink = new AjaxLink<Void>("monthlyApproveLink") {
					...
					public void onClick(AjaxRequestTarget target) {
						...
						setVisibilityOfApprovalLink(customer, this);
					}
	...
	private void setVisibilityOfApprovalLink(Customer customer, AjaxLink<Void> requestApprovalLink) {
		List<ApprovalStatus> approvalStatusForActivity = approvalStatusService.getApprovalStatusForUserWorkingForCustomer(this.user,
