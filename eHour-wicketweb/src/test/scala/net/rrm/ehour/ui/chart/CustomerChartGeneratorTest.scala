...
class CustomerChartGeneratorTest extends FunSuite with ShouldMatchers with BeforeAndAfter {
  ...
  before {
    ...
    chart = AggregateReportChartGenerators.generateCustomerReportChart("container", reportData, new EhourConfigStub())
  }
  ...
  test("should have x axis with customer categories") {
    chart should include("""xAxis:[{"categories":["Customer"]}]""")
  }
