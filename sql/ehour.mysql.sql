...
INSERT INTO `USER_ROLE` (`ROLE`, `NAME`) VALUES ('ROLE_ADMIN','Administrator'),('ROLE_CONSULTANT','Consultant'),('ROLE_PROJECTADMIN','Project Administrator'),('ROLE_REPORT','Report role');
...
INSERT INTO `USER_TO_USERROLE` (`ROLE`, `USER_ID`) VALUES ('ROLE_ADMIN',1),('ROLE_PROJECTADMIN',1);
/*!40000 ALTER TABLE `USER_TO_USERROLE` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Final view structure for view `ACTIVE_ASSIGNMENTS`
--

/*!50001 DROP TABLE IF EXISTS `ACTIVE_ASSIGNMENTS`*/;
/*!50001 DROP VIEW IF EXISTS `ACTIVE_ASSIGNMENTS`*/;
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=``@`` SQL SECURITY DEFINER */
/*!50001 VIEW `ACTIVE_ASSIGNMENTS` AS select `customer`.`CUSTOMER_ID` AS `customer_id`,`customer`.`NAME` AS `customer_name`,`project`.`PROJECT_ID` AS `project_id`,`project`.`NAME` AS `project_name`,`project`.`DEFAULT_PROJECT` AS `default_project`,`pa`.`ASSIGNMENT_ID` AS `assignment_id`,`pa`.`DEFAULT_ASSIGNMENT` AS `DEFAULT_ASSIGNMENT`,`pa`.`DATE_START` AS `date_start`,`pa`.`DATE_END` AS `date_end`,`pa`.`USER_ID` AS `user_id` from ((`CUSTOMER` `customer` join `PROJECT` `project`) join `PROJECT_ASSIGNMENT` `pa`) where ((`customer`.`ACTIVE` = _utf8'Y') and (`customer`.`CUSTOMER_ID` = `project`.`CUSTOMER_ID`) and (`project`.`ACTIVE` = _utf8'Y') and (`pa`.`ACTIVE` = _latin1'Y') and (`pa`.`PROJECT_ID` = `project`.`PROJECT_ID`)) */;
...
-- Dump completed on 2007-02-15 19:32:34
