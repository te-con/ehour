...
public class UserServiceImpl implements UserService {
	...
	public User getUser(Integer userId) throws ObjectNotFoundException {
		...
		if (user != null && user.getProjectAssignments() != null) {
			for (ProjectAssignment assignment : user.getProjectAssignments()) {
				DateRange assignmentRange = new DateRange(
						assignment.getDateStart(), assignment.getDateEnd());

				if ((!DateUtil.isDateWithinRange(currentDate, assignmentRange))
						|| (assignment.getProject() == null || !assignment
								.getProject().isActive())) {
					inactiveAssignments.add(assignment);
				}
			}
	 ...
	 * getUserAndCheckDeletability(java.lang.Integer)
	 */
	public User getUserAndCheckDeletability(Integer userId)
			throws ObjectNotFoundException {
		User user = getUser(userId);

		if ((user.getProjectAssignments() == null || user
				.getProjectAssignments().size() == 0)
				&& (user.getInactiveProjectAssignments() == null || user
						.getInactiveProjectAssignments().size() == 0)) {
			user.setDeletable(true);
		} else {
			// bummer, we need to check if the user booked any hours on the
			// assignments
			List<Integer> assignmentIds = new ArrayList<Integer>();

			assignmentIds.addAll(EhourUtil.getIdsFromDomainObjects(user
					.getProjectAssignments()));
			assignmentIds.addAll(EhourUtil.getIdsFromDomainObjects(user
					.getInactiveProjectAssignments()));
		...
		LOGGER.info("Retrieved user " + user.getUsername() + ", deletable: "
				+ user.isDeletable());
	 ...
	 * 
	 * @see
	 * net.rrm.ehour.persistence.persistence.user.service.UserService#getUser
	 * (java.lang.String)
	 */
	public User getUser(String username) {
		return userDAO.findByUsername(username);
	...
	public UserDepartment persistUserDepartment(UserDepartment department)
		...
		otherDept = userDepartmentDAO.findOnNameAndCode(department.getName(),
				department.getCode());

		if (otherDept == null) {
			userDepartmentDAO.persist(department);
		} else if (otherDept.getDepartmentId().equals(
	...
	public UserDepartment getUserDepartment(Integer departmentId)
		...
		userDepartment.setDeletable(userDepartment.getUsers() == null
				|| userDepartment.getUsers().size() == 0);
	...
	public User persistUser(User user) throws PasswordEmptyException,
		...
		if (StringUtils.isEmpty(user.getUpdatedPassword())) {
			...
			user.setSalt((int) (Math.random() * 10000));
			user.setPassword(encryptPassword(user.getUpdatedPassword(),
					user.getSalt()));
		}
		...
		if (user.getUserId() == null) {
			projectAssignmentManagementService
					.assignUserToDefaultProjects(user);
		}

		checkCustomerAssociationsAndAssignReviewerRole(user);
		userDAO.merge(user);
	 ...
	 */
	private User getUserAndAddRole(Integer userId, UserRole newRole) throws PasswordEmptyException,
			ObjectNotUniqueException {
		User user = userDAO.findById(userId);
	 ...
	 * 
	 * @see
	 * net.rrm.ehour.persistence.persistence.user.service.UserService#getUsers
	 * (net.rrm.ehour.persistence.persistence.user.domain.UserRole)
	 */
	public List<User> getUsers(UserRole userRole) {
	 ...
	 * 
	 * @see net.rrm.ehour.persistence.persistence.user.service.UserService#
	 * getUsersByNameMatch(java.lang.String, boolean,
	 * net.rrm.ehour.persistence.persistence.user.domain.UserRole)
	 */
	public List<User> getUsersByNameMatch(String match, boolean inclInactive,
			UserRole userRole) {
	 ...
	 * 
	 * @see
	 * net.rrm.ehour.persistence.persistence.user.service.UserService#deleteUser
	 ...
	 * 
	 * @see net.rrm.ehour.persistence.persistence.user.service.UserService#
	 * deleteDepartment(java.lang.Integer)
	 */
	@Transactional
	public void deleteDepartment(Integer departmentId) {
		UserDepartment department = userDepartmentDAO.findById(departmentId);
	 ...
	 */
	public void setAggregateReportService(
	...
	private void checkCustomerAssociationsAndAssignReviewerRole(User user) {
		if (user.getCustomers() != null && user.getCustomers().size() > 0) {
			UserRole customerReviewerRole = userRoleDAO
					.findById(UserRole.ROLE_CUSTOMERREVIEWER);
			user.addUserRole(customerReviewerRole);
		}
