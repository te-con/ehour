...
<div class="ContentFrame">
	<h1><fmt:message key="userReport.criteria.projectReport" />: <fmt:formatDate pattern="dd MMM yyyy" value="${report.reportCriteria.userCriteria.reportRange.dateStart}" /> - <fmt:formatDate pattern="dd MMM yyyy" value="${report.reportCriteria.userCriteria.reportRange.dateEnd}" /></h1>
		...
		<table width="100%" cellpadding="0" cellspacing="0">
			<tr>
				...
				<div class="BlueFrame">
					...
					<c:forEach items="${report.customers}" var="customer" varStatus="status">
						<tr class="customerRow" <c:if test="${status.count % 2 == 0}">style="background-color: #fefeff"</c:if>>
							<td>${customer.name}</td>
							
						<c:forEach items="${report.reportValues[customer]}" var="pag" varStatus="pagStatus">
